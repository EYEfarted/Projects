...
class MyScreenManager(ScreenManager):
    def new_results_screen(self, **kwargs):
        super(MyScreenManager, self).__init__(**kwargs)
        pn = self.get_screen('search').ids['pn'].text
        #self.res = str(query(pn))
        #name = str(time.time())
        s = ResultsScreen(name=pn)
        data_items = []
        for result in query(pn):
            data_items.append(DataItem(text=str(result)))
        ...
        list_view = ListView(adapter=list_adapter, height=self.height-30)
        layout = BoxLayout(orientation='vertical')
        layout.add_widget(list_view)
        layout.add_widget(Button(text='Return to Select Screen', on_release=self.callback))
        s.add_widget(layout)
        self.add_widget(s)
        #self.get_screen('results').ids['results_text'].text = str(query(pn))
        self.current = pn
    ...
    BoxLayout:
        ...
        BoxLayout
            ...
            Button:
                ...
                font_size: 20
                size_hint: 0.5, 1
                on_release: app.root.new_results_screen()
            Button:
